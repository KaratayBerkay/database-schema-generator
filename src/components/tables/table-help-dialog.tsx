      ...
      <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-lg border border-border bg-card shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-border bg-card text-muted-foreground transition hover:border-border hover:text-foreground"
        ...
        {helpDialog === "primaryKeys" ? (
          <div className="space-y-5 px-5 py-5">
            <p className="text-sm leading-6 text-muted-foreground">
              ...
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs text-foreground">@id</code> field.
              The options below are filtered for the active database provider.
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
              {pkTypes.map((type) => (
                <div key={type.value} className="rounded-md border border-border bg-background p-3">
                  <div className="flex items-center justify-between gap-3">
                    <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-semibold ${type.badgeClass}`}>
                      {type.value}
                    </span>
                    <span className="text-xs font-medium text-muted-foreground">{providerDisplay}</span>
                  </div>
                  <p className="mt-2 text-sm text-muted-foreground">{type.summary}</p>
                  <code className="mt-3 block overflow-x-auto whitespace-nowrap rounded bg-card px-2 py-2 font-mono text-xs text-foreground">
                    {pkExampleLine(pkName, type.value, activeProvider)}
                  </code>
                </div>
              ))}
            </div>
            <p className="rounded-md border border-amber-500/30 bg-amber-500/15 px-3 py-2 text-sm leading-6 text-amber-200">
              Postgres UUID primary keys use the database generator by default:{" "}
              <code className="ml-1 rounded bg-card px-1 py-0.5 font-mono text-xs text-amber-200">gen_random_uuid()</code>.
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-md border border-border bg-background p-3">
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Use <code className="rounded bg-card px-1 py-0.5 font-mono text-xs text-foreground">id</code> for new tables unless you are matching an existing database.
                </p>
              </div>
              <div className="rounded-md border border-border bg-background p-3">
                <p className="text-sm font-semibold text-foreground">Allowed format</p>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Start with a letter. Use only letters, numbers, and underscores. Prefer camelCase.
                </p>
              </div>
              <div className="rounded-md border border-border bg-background p-3">
                <p className="text-sm font-semibold text-foreground">Legacy schemas</p>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Names like <code className="rounded bg-card px-1 py-0.5 font-mono text-xs text-foreground">userId</code> are fine when the source schema already uses them.
                </p>
              </div>
              <div className="rounded-md border border-border bg-background p-3">
                <p className="text-sm font-semibold text-foreground">Generated line</p>
